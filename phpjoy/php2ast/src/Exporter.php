...
abstract class Exporter
{
    ...
    const JEXP_FORMAT = 1;
    /** Constant for Neo4J Admin import format (to be used with neo4j-admin-import) */
    const NEO4J_ADMIN_FORMAT = 3;
     ...
     *
     * @param $ast       ast\Node The AST to export.
     ...
     *         function was called.)
     */
    public function export($ast, $funcid, $nodeline = 0, $childname = "", $childnum = 0, $namespace = "", $uses = [], $classname = "", $flag_classid = 0): int
    {
        // TODO $a = "1".
        //                "2" 生成的时候加上ROOT_NODE标签
        ...
        if ($ast instanceof ast\Node) {
            ...
            if ($ast->kind === ast\AST_CLASS) {
                $classname = $nodename;
                $flag_classid = $this->nodecount;
            }

            // store node, export all children and store the relationships
            $rootnode = $this->store_node(self::LABEL_AST, $nodetype, $nodeflags, $nodeline, null, $childnum, $funcid, $classname, $this->quote_and_escape($namespace), $nodeendline, $nodename, $nodedoccomment, $this->getFileId(), $flag_classid);
            ...
            if ($ast->kind === ast\AST_FUNC_DECL || $ast->kind === ast\AST_METHOD || $ast->kind === ast\AST_CLOSURE) {
                $funcid = $rootnode;
                $entrynode = $this->store_node(self::LABEL_ART, self::FUNC_ENTRY, null, null, null, null, $rootnode, $classname, $this->quote_and_escape($namespace), null, $this->quote_and_escape($nodename), null, $this->getFileId(), $flag_classid);
                $exitnode = $this->store_node(self::LABEL_ART, self::FUNC_EXIT, null, null, null, null, $rootnode, $classname, $this->quote_and_escape($namespace), null, $this->quote_and_escape($nodename), null, $this->getFileId(), $flag_classid);
            ...
            foreach ($ast->children as $childrel => $child) {
                ...
                if ($ast->kind === ast\AST_CLASS && $childrel === "stmts") {
                    ...
                    $childnode = $this->export($child, $tnode, $nodeline, "stmts", 0, $namespace, $uses, $classname, $flag_classid);
                ...
                elseif ($ast->kind === ast\AST_NAME && $childrel === "name" && $ast->flags !== ast\flags\NAME_FQ) {
                    ...
                    $childnode = $this->export($child, $funcid, $nodeline, $childrel, $i, $namespace, $uses, $classname, $flag_classid);
...
//        }
        ...
        // it and put quotes around the content
            ...
            $rootnode = $this->store_node(self::LABEL_AST, $nodetype, null, $nodeline, $this->quote_and_escape($ast), $childnum, $funcid, $classname, $this->quote_and_escape($namespace), null, null, null, $this->getFileId(), $flag_classid);
        }
        ...
        // a class that does not use an "extends" or "implements" statement, a function that has no return value, etc.
            ...
            $rootnode = $this->store_node(self::LABEL_AST, $nodetype, null, $nodeline, null, $childnum, $funcid, $classname, $this->quote_and_escape($namespace), null, null, null, $this->getFileId(), $flag_classid);
        }
        ...
        // We print these similarly as strings, but without quotes.
            ...
            $nodecode = (string)$ast;
            $rootnode = $this->store_node(self::LABEL_AST, $nodetype, null, $nodeline, $nodecode, $childnum, $funcid, $classname, $this->quote_and_escape($namespace), null, null, null, $this->getFileId(), $flag_classid);
        }
    ...
    private function applyTranslationRulesForName($haystack, $uses): string
    {
        ...
        foreach ($uses as $needle => $replacement) {
            ...
            if (substr($haystack, 0, strlen($needle)) === $needle) {
                ...
                if ($old_needle === $haystack) {
                    return $old_replacement;
                }
            }
        }
     ...
     *
     * @param $filename string The file's name, which will be stored under the
     *                  'name' poperty of the File node.
     *
     * @return integer The index of the stored file node.
     */
    public function store_filenode($filename): int
    {


        $fnode = $this->store_node(self::LABEL_FS, self::FILE, null, null, null, null, null, null, null, null, $this->quote_and_escape($filename), null, null);
        $this->setFileId($fnode);
        return $fnode;
    }
    ...
    public function store_toplevelnode($flag, $name, $lineno, $endlineno, $childnum = null, $funcid = null, $namespace = null): int
    {
        ...
        $tnode = $this->store_node(self::LABEL_AST, self::TOPLEVEL, $flag, $lineno, null, $childnum, $funcid, null, $this->quote_and_escape($namespace), $endlineno, $this->quote_and_escape($name), null, $fileid);
        ...
        $entrynode = $this->store_node(self::LABEL_ART, self::FUNC_ENTRY, null, null, null, null, $tnode, null, null, null, $this->quote_and_escape($name), null, $fileid);
        $exitnode = $this->store_node(self::LABEL_ART, self::FUNC_EXIT, null, null, null, null, $tnode, null, null, null, $this->quote_and_escape($name), null, $fileid);
    ...
    public function store_dirnode($filename): int
    {
        ...
        return $this->store_node(self::LABEL_FS, self::DIR, null, null, null, null, null, null, null, null, $this->quote_and_escape($filename), null, $this->getFileId());
    }
     ...
     *
     * @param $str  string The string to be quoted and escaped
     *
     ...
     *
     * @return int The number of nodes created so far.
     */
